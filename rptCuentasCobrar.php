...
echo "<h1>Reporte de Cuentas x Cobrar</h1>
...
from `salida_almacenes` s, clientes c where s.`monto_final` >
      (
        select COALESCE(sum(cbd.monto_detalle), 0)
         ...
         and cb.cod_estado<>2 and cb.fecha_cobro between '$fecha_iniconsulta' and
               '$fecha_finconsulta'
      ) and s.`cod_cliente` = c.`cod_cliente` and c.cod_area_empresa='$rpt_territorio' and 
      s.`salida_anulada` = 0 and s.cod_tiposalida=1001 and 
...
echo "<br><table cellspacing='0' border='0' align='center' class='texto' width='100%'>
<tr>
<th>N.R.</th>
<th>Fecha</th>
<th>Cliente</th>
